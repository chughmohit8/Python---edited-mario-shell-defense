...
def gameOver():
    ...
    ### Find the time from beginning to end
    end = time.time()
    totaltime = end - start
    ...
    ### Create the time font and positioning
    timecaption = font.render("Time: " + str(totaltime), True, TEXT_COLOR)
    timecaptionpos = caption.get_rect(midtop = scorepos.midbottom )
    
    background.blit(background_orig, (0,0))
    background.blit(caption, captionpos)
    background.blit(scoreline, scorepos)
    ### Add the time caption at end screen
    background.blit(timecaption, timecaptionpos)
...
def main():
    ...
    enemycounter = 150
    playing = True
    ### Start a timer to keep track of until Mario dies
    global start
    # main game loop
