    ...
    def update(self):
        ...
        if keys[pygame.K_e]:
            if not self.fireball_lock and Fireball.count <= MARIO_MAX_FIREBALLS :
                ...
                ### For the third parameter, made False for red fireball
                FIREBALLS.add( Fireball( pos, self.walkingRight,False) )
        ### Added key 'f' to shoot a blue fireball
            ...
            if not self.fireball_lock and Fireball.count <= MARIO_MAX_FIREBALLS :
                ...
                if not self.walkingRight:
                    pos = self.rect.midleft
                ### For the third parameter, made True for blue fireball
                FIREBALLS.add( Fireball( pos, self.walkingRight, True) )
...
class Fireball(pygame.sprite.Sprite):
    ...
    ###Added a blue parameter, so when initiating you can decide whether it will be a blue (True) or red (False) fireball.
    ...
    def _move(self):
        ...
        ### When the fireball is blue, utilizes the blue fireball's grav instead of the old default one. (Now only red)
        if self.blue:
            self.vel_y += BLUE_FIREBALL_GRAV
        else:
